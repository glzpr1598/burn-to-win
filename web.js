...
app.use((req, res, next) => {
    res.locals.stylePlayerName = (playerName, genderMap) => {
        if (!playerName || !genderMap) return playerName || '';
...
app.get('/api/special-match/team-schedule-matches/:scheduleId/:teamName', async (req, res) => {
    try {
        ...
        // 1. 팀에 속한 멤버들을 가져옵니다.
        const [teamMembersResult] = await pool.query('SELECT member_name FROM match_team_member WHERE team_id = (SELECT id FROM match_team WHERE schedule_id = ? AND team_name = ?)', [scheduleId, teamName]);
...
// API: 팀 삭제
app.post('/api/admin/delete-team', isAuthenticated, async (req, res) => {
    const { teamId } = req.body;

    try {
        await pool.query('DELETE FROM match_team_member WHERE team_id = ?', [teamId]);
        await pool.query('DELETE FROM match_team WHERE id = ?', [teamId]);
...
app.post('/api/admin/team-members', isAuthenticated, async (req, res) => {
    ...
    try {
        const [result] = await pool.query('INSERT INTO match_team_member (team_id, member_name) VALUES (?, ?)', [teamId, memberName]);
...
// API: 팀에서 멤버 삭제
app.post('/api/admin/delete-team-member', isAuthenticated, async (req, res) => {
    const { memberId } = req.body;

    try {
        await pool.query('DELETE FROM match_team_member WHERE id = ?', [memberId]);
    ...
    try {
        ...
        if (schedule_id) {
            ...
            if (teams.length > 0) {
                ...
                const [members] = await pool.query('SELECT team_id, member_name FROM match_team_member WHERE team_id IN (?) ORDER BY member_name ASC', [teamIds]);
                members.forEach(m => {
...
app.post('/api/special-match/record', async (req, res) => {
    ...
    if (!schedule_id || !round_num || !court_num || !team1_deuce || !team2_deuce) {
        return res.status(400).json({ success: false, message: '필수 정보(일정, 라운드, 코트, 듀스코트 선수)가 누락되었습니다.' });
    }
    
    try {
        let team1_result, team2_result;
        if (Number(team1_score) > Number(team2_score)) {
            team1_result = '승'; team2_result = '패';
        } else if (Number(team1_score) < Number(team2_score)) {
            team1_result = '패'; team2_result = '승';
        } else {
            team1_result = '무'; team2_result = '무';
        }
        ...
        const params = [
            id || null, schedule_id, round_num, court_num, 
        ...
        await pool.query(sql, params);
        res.json({ success: true, message: '경기 기록이 저장되었습니다.' });

    } catch (err) {
        console.error('[/api/special-match/record] 에러:', err.message);
...
app.delete('/api/special-match/record', async (req, res) => {
    const { id } = req.body;
    if (!id) {
        return res.status(400).json({ success: false, message: '삭제할 ID가 없습니다.' });
    }
    try {
        await pool.query('DELETE FROM matchrecord WHERE id = ?', [id]);
        res.json({ success: true, message: '삭제되었습니다.' });
    } catch (err) {
        console.error('[/api/special-match/record] DELETE 에러:', err.message);
...
app.post('/api/special-match/videographer', async (req, res) => {
    const { id, schedule_id, round_num, court_num, videographer } = req.body;
     ...
     if (!schedule_id || !round_num || !court_num) {
        return res.status(400).json({ success: false, message: '필수 정보가 누락되었습니다.' });
    }
